...
def _game_filter(game_ids):
    """Return (sql_clause, params) restricting to game_ids, or empty if None/all."""
    if not game_ids:
        return "", ()
...
def usage_rate(player_id, team_id, game_ids=None, events=None,
    ...
    """
    USG% = 100 * (FGA + TOV) * (total game minutes)
    ...
    if player_min is None:
        player_min = minutes_played(game_ids).get(player_id, 0.0)
    if game_min is None:
        game_min = total_game_minutes(game_ids)
    if player_min <= 0:
        return None
