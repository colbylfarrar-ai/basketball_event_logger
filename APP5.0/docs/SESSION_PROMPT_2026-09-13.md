...
>
> * **Lab lost a level.** `lab_adv_sub` is gone; `lab_sub` is now
>   `Efficiency & DNA · Résumé & Form · Game Flow · Impact Lab ·
>   Chart Builder`. Any pointer at "Lab → Advanced" is stale.
> * **Two more `st.tabs` became `_sub_seg`** (Quarters `ch_q_sub`, Offense ▸
>   Shooting `ch_sh_sub`) and Trends gained `ch_tr_sub`.
> * **`_jump` takes a subview path**, and the Insights section switcher
>   consumes a parked step, so a cross-link can name a section.
> * **Percentiles state their pool** in these surfaces;
>   `tracker/test_pctile_pool.py` now fails a NEW bare one and carries the
>   four remaining files as a named allowlist (`scout_tab`, `player_card`,
