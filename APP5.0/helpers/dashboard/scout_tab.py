...
#: The section a coach picks between, in the order a scout is read: the plan
#: first, then the people, then what they do, then where they shoot, then the
#: engine reads, then the paper. The old group dividers named these same seven
#: groups — they were just all rendered at once.
SECTION_LABELS = ["Call sheet", "Personnel", "Team profile", "Sets & schemes",
                  "Situational & shooting", "Engine reads",
                  "Deep splits & print"]
...
def _sheet_download(sc, opp_label, extra, prefs, *, key):
    ...
    # `extra` may be a zero-arg builder, and that is the shape to prefer — it is
    # the expensive half of both documents (a league xPP-Q fit, every saved play
    # re-rendered to SVG, thirteen engines walked) and neither document exists
    # until a coach presses Prepare. Memoized so the two downloads share one
    # build rather than paying for it twice.
    _built = {}

    def _extra():
        if "v" not in _built:
            _built["v"] = extra() if callable(extra) else extra
        return _built["v"]

    # The one-page call sheet first — it is the artifact a coach actually
    # carries. The full sheet below is the appendix behind it.
    pdf_or_html_download(
        "Call sheet (1 page)",
        lambda: SC.call_sheet_html(sc, opp_label, extra=_extra(),
                                   plan_lines=_extra().get("game_plan")),
        f"callsheet_{sc['name'].replace(' ', '_')}", key=f"{key}_call",
        fp=(sc["name"], tuple(sorted(prefs["print"]))))
    _fp = (tuple(sorted(prefs["print"])), prefs["layout"], prefs["diagrams"],
           prefs["shotwall"])
    pdf_or_html_download(
        "Scout sheet",
        lambda: SC.printable_html(
            sc, opp_label, hidden=prefs["print"], extra=_extra(),
...
def render(ctx):
    ...
    # ══════════════════════════════════════════════════════════════════
    #  THE SECTIONS — one open at a time
    # ══════════════════════════════════════════════════════════════════
    # `_seg`, NOT the old linear scroll: every one of these ~35 blocks used
    # to render on every rerun, gated only by _show(), and most coaches
    # leave most toggles on — so nearly all of the work was done for every
    # interaction. Insights moved off six eager st.tabs for exactly this
    # reason and its per-rerun cost went DOWN as its section count went up.
    _sec = SD.section_picker(SECTION_LABELS)

    # Read by the Keys section AND by the printable's extra block, so it is
    # computed once out here rather than leaked across a lazy section boundary.
    ...
    if _sec == "Call sheet":
        ...
        if not _self and _opp_tid is not None and ctx.has_tracked and _show("game_plan"):
            SD.render_game_plan(ctx, _my_team_id, my_game_ids=_my_gp)
    ...
    if _sec == "Deep splits & print":
        ...
        def _build_extra():
            """Everything the sheet needs that the engine does not own.

            A CALLABLE, not a dict: this used to be assembled on every rerun of
            the tab, for a document nobody had asked for yet. `_concession`
            fits a league xPP-Q model, `_saved_plays_for_sheet` re-renders every
            saved play to SVG and the engine reads walk thirteen engines — all
            of it for a Prepare button that might never be pressed."""
            return {
            "breakeven": {
            ...
            # print parity: the SAME verdict tuples the tab renders in the Call
            # sheet section. Rebuilt here rather than leaked across the lazy
            # section boundary — a coach who never opened that section still
            # gets the plan on the paper.
            "game_plan": SD.game_plan_print(
        ...
        st.markdown("<div class='lab-hdr'>Printable scout sheet</div>",
                    unsafe_allow_html=True)
        _sheet_download(sc, opp_label, _build_extra, _prefs, key="scout_dl")
