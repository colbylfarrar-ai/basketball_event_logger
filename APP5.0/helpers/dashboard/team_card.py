...
def _banner_html(*, team_id, gender, season, power, rank, pool_n, wins, losses,
    ...
    if _pw is not None and _lw is not None and _mg >= 6:
        ...
        _luck_bit = (
            ...
            f"prediction. The exponent SATURATES: a team outscoring its "
            f"schedule by a wide enough margin expects almost every game, so "
            f"an expectation pinned at the games played is the formula "
            f"behaving normally, not a bug.'>"
