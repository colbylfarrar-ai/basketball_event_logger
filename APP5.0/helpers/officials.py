...
def _clutch_and_playtype(game_ids, exclude=None):
    ...
    `exclude` is a set of foul event ids to leave out of every tally — the live
    pool the rating is built on (no intentional clock-stops, nothing after the
    game was decided). A trailing team hacking in the last minute is not a
    whistle profile, and it lands squarely inside CLUTCH_MARGIN if left in.
    It is a SUPERSET of the strategic fouls this function drops on its own, so
    callers that pass nothing still get the clock-stop exclusion."""
    ...
    for gid, evs in by_game.items():
        ...
        for e in evs:
            et = e["event_type"]
            if et == "foul" and e["id"] not in _strategic and e["id"] not in skip:
                q = e["quarter"] or 0
                opk = e["official_id"]
...
def official_ratings(gender=None, game_ids=None, season="Current", scored=None):
    ...
    which SETS they whistle more than the field (pt_bias) and which DEFENSES
    they whistle more (def_bias).
    ...
    {"officials": [row + {leverage, clutch, clutch_pg, pt_bias, def_bias,
    ...
    clutch, pt_off, pt_lg, df_off, df_lg = _clutch_and_playtype(
        list(games.keys()), exclude=dropped)
    ...
    for r in rows:
        ...
        # Both bias axes are gated at RATING_MIN_GAMES (N3): the table used to
        # list anyone with two calls on a tag, which at one game worked is a
        # coin toss with a label.
        _rateable = r["games"] >= RATING_MIN_GAMES
        r["pt_bias"] = _share_bias(pt_off.get(opk, {}) if _rateable else {},
                                   pt_lg, lg_pt_total)
        ...
        r["def_bias"] = _share_bias(df_off.get(opk, {}) if _rateable else {},
                                    df_lg, lg_df_total)
