...
from helpers.ui import page_chrome, page_header, empty_state, seg as _seg
...
page_header("Event Editor",
            sub="Correct, delete or insert any play-by-play event. Edits re-derive "
                "+/- for changed baskets and keep lineup stats valid; deletes cascade "
                "their on-court snapshot; an insert clones the on-floor five from the "
                "nearest logged event. Pick a tool below.")
...
#  ONE TOOL AT A TIME
# ══════════════════════════════════════════════════════════════════════════════
# This page had five tools stacked down one scroll, and its namesake — the grid
# that fixes an event — sat roughly 380 lines below the fold, behind a missed-sub
...
# and every rerun (each Save fires one) rebuilt all five: the insert form alone
# is twenty-odd widgets, and the lineup fixer runs its own event load plus a
# floor-run query.
#
# A keyed segmented control, and an if-dispatch — the same lazy-section contract
# the Input Hub and the Insights deck use (`_seg`, never `st.tabs`, which snaps
# back to the first tab on every rerun). Only the chosen tool's queries run, the
# grid leads because it is what the page is for, and the selection survives a
# Save.
#
# The filters move INSIDE the tools they actually govern. They used to sit above
...
#: Tools that read the FILTERED event list. The other two are whole-game by
#: nature: an insert has no existing event to filter, and a missed substitution
#: propagates forward across quarters.
_EE_FILTERED = ("Fix events", "Shot locations", "Bulk tags")

# One load for the whole page. The lineup fixer and the coverage chips each used
# to run their own `load_events(gid, None)`, which is the same query twice.
_all_ev = EL.load_events(gid, None)

# One-tap fix on a phone, courtside: the tracker PWA has its own event editor
# (Edit log, on the game screen and on the setup screen) with a tap-to-open form
# per event, offline-tolerant. It edits ONE event at a time — the bulk tools, the
# court fixer, the insert and the lineup correction are only here.
st.caption("Fixing a single event courtside? The tracker app's **Edit log** does "
           "that on a phone. This page is for everything a phone can't: bulk "
           "re-tagging, moving a shot on the court, inserting a missed event and "
