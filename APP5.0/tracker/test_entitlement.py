...
print("free_demo_game_id / can_see_tracked_game_view (the first-game-free demo)")
ok(E.free_demo_game_id(free) == g1, "free coach's demo = own team's earliest game (g1)")
ok(E.free_demo_game_id(solo) is None, "paid coach has no demo restriction (None)")
ok(E.free_demo_game_id(admin) is None, "admin has no demo restriction (None)")
ok(E.free_demo_game_id(None) is None, "no identity -> no demo")
ok(E.free_demo_game_id({"role": "coach", "plan": "free", "team_id": None}) is None,
   "free coach with no team -> no demo")
ok(E.can_see_tracked_game_view(free, g1), "free coach sees their demo game's depth")
ok(not E.can_see_tracked_game_view(free, g2), "free coach blocked on a non-demo game")
ok(E.can_see_tracked_game_view(solo, g2), "paid coach sees any game's depth")
